...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    if (hideDefaultCursor) {
      ...
      // Make sure interactive elements under cursor-target don't show cursor
    ...
    const scrollHandler = () => {
      ...
      if (!isStillOverTarget) {
        if (currentLeaveHandler) {
          (currentLeaveHandler as EventListener)(new Event('mouseleave'));
        }
      }
    };
    ...
    return () => {
      ...
      // Clean up the style injection
      const styleNodes = document.head.querySelectorAll('style');
      styleNodes.forEach(node => {
        if (node.innerHTML.includes('cursor: none !important')) {
            node.remove();
        }
      });
  ...
  return (
    <div ref={cursorRef} className="target-cursor-wrapper" style={{ opacity: 0 }}>
      {/* Black cursor for visibility over light areas instead of difference blend mode which was acting weird */}
